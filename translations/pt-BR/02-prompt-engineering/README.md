...
- [O Que Você Aprenderá](../../../02-prompt-engineering)
...
- [Capturas de Tela do Aplicativo](../../../02-prompt-engineering)
...
<img src="../../../translated_images/pt-BR/what-youll-learn.c68269ac048503b2.webp" alt="O Que Você Aprenderá" width="800"/>

No módulo anterior, você viu como a memória possibilita IA conversacional e usou os Modelos do GitHub para interações básicas. Agora focaremos em como fazer perguntas — os próprios prompts — usando o GPT-5.2 do Azure OpenAI. A forma como você estrutura seus prompts afeta dramaticamente a qualidade das respostas. Começamos revisando as técnicas fundamentais de prompt, depois passamos para oito padrões avançados que aproveitam ao máximo as capacidades do GPT-5.2.

Usaremos o GPT-5.2 porque ele introduz controle de raciocínio — você pode dizer ao modelo a quantidade de pensamento a ser feita antes de responder. Isso torna as estratégias de prompting mais evidentes e ajuda a entender quando usar cada abordagem. Também nos beneficiaremos dos limites de taxa mais baixos do Azure para GPT-5.2 em comparação com os Modelos GitHub.
...
> **Nota:** Se você não completou o Módulo 01, siga as instruções de implantação previstas lá primeiro.
...
Engenharia de prompt é sobre desenhar texto de entrada que consistentemente gera os resultados necessários. Não é só fazer perguntas — trata-se de estruturar solicitações para que o modelo entenda exatamente o que você quer e como entregar.

Pense nisso como dar instruções para um colega. "Corrija o bug" é vago. "Corrija a exceção de ponteiro nulo em UserService.java linha 45 adicionando uma verificação nula" é específico. Modelos de linguagem funcionam do mesmo jeito — especificidade e estrutura importam.

<img src="../../../translated_images/pt-BR/how-langchain4j-fits.dfff4b0aa5f7812d.webp" alt="Como LangChain4j se Encaixa" width="800"/>

LangChain4j fornece a infraestrutura — conexões de modelo, memória e tipos de mensagem — enquanto padrões de prompt são apenas texto cuidadosamente estruturado que você envia através dessa infraestrutura. Os blocos principais são `SystemMessage` (que define o comportamento e papel da IA) e `UserMessage` (que carrega sua solicitação real).
...
Antes de mergulhar nos padrões avançados deste módulo, vamos rever cinco técnicas fundamentais de prompting. São os blocos básicos que todo engenheiro de prompt deveria conhecer. Se você já trabalhou com o [módulo Quick Start](../00-quick-start/README.md#2-prompt-patterns), já viu esses em ação — aqui está a estrutura conceitual por trás deles.

### Prompt Zero-Shot

A abordagem mais simples: dê uma instrução direta ao modelo sem exemplos. O modelo depende totalmente do seu treinamento para entender e executar a tarefa. Funciona bem para pedidos diretos onde o comportamento esperado é óbvio.
...
**Quando usar:** Classificações simples, perguntas diretas, traduções ou qualquer tarefa que o modelo possa processar sem orientação adicional.

### Prompt Few-Shot

Forneça exemplos que demonstrem o padrão que você quer que o modelo siga. O modelo aprende o formato esperado de entrada-saída pelos seus exemplos e o aplica a novas entradas. Isso melhora dramaticamente a consistência para tarefas cujo formato ou comportamento desejado não é óbvio.

<img src="../../../translated_images/pt-BR/few-shot-prompting.9d9eace1da88989a.webp" alt="Prompt Few-Shot" width="800"/>

*Aprendendo com exemplos — o modelo identifica o padrão e aplica a novas entradas*
...
Peça ao modelo para mostrar seu raciocínio passo a passo. Ao invés de ir direto para uma resposta, o modelo quebra o problema e trabalha explicitamente cada parte. Isso melhora a precisão em tarefas de matemática, lógica e raciocínio com múltiplas etapas.
...
**Quando usar:** Problemas matemáticos, puzzles lógicos, debug ou qualquer tarefa onde mostrar o processo melhora precisão e confiança.
...
**Quando usar:** Revisões de código, tutoria, análise específica de domínio ou quando você precisa de respostas adaptadas a um nível de especialização ou perspectiva específica.

### Templates de Prompt

Crie prompts reutilizáveis com espaços reservados variáveis. Ao invés de escrever um novo prompt toda vez, defina um template uma vez e preencha valores diferentes. A classe `PromptTemplate` do LangChain4j facilita isso com a sintaxe `{{variable}}`.

<img src="../../../translated_images/pt-BR/prompt-templates.14bfc37d45f1a933.webp" alt="Templates de Prompt" width="800"/>

*Prompts reutilizáveis com espaços reservados variáveis — um template, muitos usos*
...
**Quando usar:** Consultas repetidas com entradas diferentes, processamento em lote, construção de fluxos de trabalho de IA reutilizáveis ou qualquer cenário onde a estrutura do prompt é a mesma mas os dados mudam.

---

Esses cinco fundamentos dão a você uma caixa de ferramentas sólida para a maioria das tarefas de prompting. O restante deste módulo se baseia nelas com **oito padrões avançados** que aproveitam o controle de raciocínio do GPT-5.2, autoavaliação e capacidades de saída estruturada.

## Padrões Avançados

Com os fundamentos cobertos, vamos para os oito padrões avançados que tornam este módulo único. Nem todos os problemas precisam da mesma abordagem. Algumas perguntas precisam de respostas rápidas, outras de pensamento profundo. Algumas precisam de raciocínio visível, outras só resultados. Cada padrão abaixo é otimizado para um cenário diferente — e o controle de raciocínio do GPT-5.2 torna as diferenças ainda mais marcantes.
...
*O controle de raciocínio do GPT-5.2 permite especificar quanto pensamento o modelo deve fazer — de respostas rápidas e diretas a exploração profunda*
...
> - "Qual é a diferença entre os padrões de prompting de baixa e alta disposição?"
...
**Alta Disposição (Profundo & Completo)** - Para problemas complexos onde você quer análise abrangente. O modelo explora detalhadamente e mostra raciocínio detalhado. Use para design de sistemas, decisões arquitetônicas ou pesquisas complexas.
...
**Execução de Tarefas (Progresso Passo a Passo)** - Para fluxos de trabalho com múltiplas etapas. O modelo fornece um plano inicial, narra cada passo conforme trabalha, depois dá um resumo. Use para migrações, implementações ou qualquer processo com múltiplas etapas.
...
O prompting em Cadeia de Pensamento pede explicitamente que o modelo mostre seu processo de raciocínio, melhorando a precisão para tarefas complexas. A divisão passo a passo ajuda humanos e IA a entenderem a lógica.
...
> - "Como posso capturar e exibir atualizações intermediárias de progresso em uma UI?"

<img src="../../../translated_images/pt-BR/task-execution-pattern.9da3967750ab5c1e.webp" alt="Padrão de Execução de Tarefa" width="800"/>

*Fluxo de trabalho Planejar → Executar → Resumir para tarefas com múltiplas etapas*

**Código Auto-Reflexivo** - Para gerar código com qualidade de produção. O modelo gera código seguindo padrões de produção com tratamento adequado de erros. Use ao construir novos recursos ou serviços.
...
*Loop de aprimoramento iterativo - gerar, avaliar, identificar problemas, melhorar, repetir*

**Análise Estruturada** - Para avaliação consistente. O modelo revisa código usando um framework fixo (correção, práticas, desempenho, segurança, manutenção). Use para revisões de código ou avaliações de qualidade.
...
> - "Como posso customizar o framework de análise para diferentes tipos de revisão de código?"
> - "Qual é a melhor forma de analisar e atuar sobre a saída estruturada programaticamente?"
> - "Como assegurar níveis de severidade consistentes em diferentes sessões de revisão?"
...
**Chat de Múltiplas Rodadas** - Para conversas que precisam de contexto. O modelo lembra mensagens anteriores e constrói a partir delas. Use para sessões interativas de ajuda ou Q&A complexos.
...
**Raciocínio Passo a Passo** - Para problemas que requerem lógica visível. O modelo mostra raciocínio explícito para cada passo. Use para problemas matemáticos, puzzles lógicos ou quando precisar entender o processo de pensamento.
...
**Saída Constrangida** - Para respostas com requisitos específicos de formato. O modelo segue rigorosamente regras de formato e comprimento. Use para resumos ou quando precisar de estrutura precisa na saída.
...
A interface web permite que você experimente diferentes estratégias de prompting. Cada padrão resolve diferentes problemas - experimente para ver quando cada abordagem se destaca.

### Baixo vs Alto Entusiasmo

Faça uma pergunta simples, como "Qual é 15% de 200?" usando Baixo Entusiasmo. Você obterá uma resposta instantânea e direta. Agora pergunte algo complexo, como "Desenhe uma estratégia de cache para uma API de alto tráfego" usando Alto Entusiasmo. Observe como o modelo desacelera e fornece um raciocínio detalhado. Mesmo modelo, mesma estrutura de pergunta - mas o prompt indica o quanto pensar.
...
Fluxos de trabalho multi-etapas se beneficiam de planejamento prévio e narração do progresso. O modelo descreve o que vai fazer, narra cada passo, e então resume os resultados.
...
Tente "Criar um serviço de validação de e-mail". Em vez de apenas gerar código e parar, o modelo gera, avalia com base em critérios de qualidade, identifica fraquezas e melhora. Você verá ele iterar até que o código cumpra os padrões de produção.
...
Pergunte "O que é Spring Boot?" e logo depois siga com "Mostre um exemplo". O modelo lembra a sua primeira pergunta e lhe dá um exemplo específico de Spring Boot. Sem memória, essa segunda pergunta seria muito vaga.
...
Escolha um problema de matemática e experimente tanto o Raciocínio Passo a Passo quanto o Baixo Entusiasmo. Baixo entusiasmo só lhe dá a resposta - rápido, porém opaco. Passo a passo mostra cada cálculo e decisão.
...
Quando você precisa de formatos específicos ou contagem de palavras, este padrão impõe aderência rigorosa. Tente gerar um resumo com exatamente 100 palavras em formato de tópicos.
...
GPT-5.2 permite controlar o esforço computacional através dos seus prompts. Baixo esforço significa respostas rápidas com exploração mínima. Alto esforço significa que o modelo leva tempo para pensar profundamente. Você está aprendendo a combinar esforço com a complexidade da tarefa - não perca tempo em perguntas simples, mas também não acelere decisões complexas.

**Estrutura Guia o Comportamento**

Percebe as tags XML nos prompts? Elas não são decorativas. Modelos seguem instruções estruturadas com mais confiabilidade do que texto livre. Quando você precisa de processos multi-etapas ou lógica complexa, a estrutura ajuda o modelo a acompanhar onde está e o que vem a seguir.
